...
translations = {
    ...
    "À quel point cette séance/course a-t-elle été physiquement exigeante ?": {
        "Français": "À quel point cette séance/course a-t-elle été physiquement exigeante ?",
        "English": "How physically demanding was this session/race?"
    },
    "À quel point cette séance/course a-t-elle été mentalement exigeante ?": {
        "Français": "À quel point cette séance/course a-t-elle été mentalement exigeante ?",
        "English": "How mentally demanding was this session/race?"
    },
    "Comment évalues-tu globalement la difficulté de cette séance/course ?": {
        "Français": "Comment évalues-tu globalement la difficulté de cette séance/course ?",
        "English": "How do you rate the overall difficulty of this session/race?"
    },
    "Commentaires": {
...
def show_explanation():
    ...
    st.subheader("**Présentation du suivi du RPE et du CR10 en football**")
    st.write("L’évaluation de l’effort perçu est un outil utilisé depuis plusieurs années dans le sport de haut niveau.")
    st.subheader("**Origine et principe du RPE et du CR10**")
    st.write("L’échelle de perception de l’effort (RPE) a été développée par le physiologiste suédois Gunnar Borg dans les années 1970. Initialement conçue pour refléter l’intensité de l’effort en lien avec la fréquence cardiaque, elle est utilisée dans de nombreux sports pour évaluer la charge d’entraînement. L’échelle RPE 6-20 est graduée de 6 à 20, ces valeurs ayant été choisies pour correspondre approximativement à la fréquence cardiaque multipliée par 10 chez un adulte en bonne santé. Ainsi, un effort ressenti à 13 (effort 'assez dur') correspond en moyenne à une fréquence cardiaque de 130 battements par minute. L’échelle CR10, une version simplifiée, va de 0 à 10. Elle est plus intuitive et plus facile à utiliser au quotidien. Dans le football, elle est souvent privilégiée, car elle permet aux joueurs de donner une estimation rapide et claire de leur ressenti après un match ou une séance d’entraînement.")

    st.subheader("**Comment utiliser le RPE ou le CR10 en aviron ?**")
    st.write("À la fin d’une séance d’entraînement ou d’une course, chaque athlète est invité à donner une note sur l’échelle choisie en répondant à une question simple : 'Comment as-tu ressenti l’intensité de cette séance ?' Le joueur doit donner une réponse en fonction de son ressenti global, sans se baser uniquement sur sa fatigue musculaire ou son essoufflement. L’idée est d’avoir une vision subjective de la difficulté de la séance, intégrant à la fois la charge physique, la charge mentale et les exigences du jeu. Cette évaluation est ensuite multipliée par la durée de l’effort en minutes pour obtenir une valeur de charge d’entraînement.")

    st.subheader("**Exemple de calcul :**")
    st.write("Un athlète attribue une note de 7 sur le CR10 après un match de 90 minutes. La charge d’entraînement est donc : 7 × 90 = 630. Ce chiffre permet de comparer les charges de travail entre les joueurs et sur différentes périodes.")

    st.subheader("**Traitement et analyse des données**")
    st.write("Les valeurs de charge d’entraînement recueillies sont enregistrées et permettent d’analyser l’évolution de la charge de travail au fil des semaines. Un suivi régulier permet d’identifier des tendances et d’adapter la préparation physique en conséquence. L’un des indicateurs clés utilisés est le ratio charge aiguë / charge chronique (ACWR). Il compare la charge de travail récente (dernière semaine) à la charge moyenne des quatre semaines précédentes.")
    st.write("- Un ratio trop élevé (> 1,5) signifie que la charge récente est bien supérieure à la charge habituelle, augmentant le risque de blessure.")
    st.write("- Un ratio trop faible (< 0,8) indique une charge insuffisante, ce qui peut limiter la progression du joueur.")
    st.write("Les données peuvent être utilisées individuellement ou collectivement.")

    st.subheader("**Pourquoi ces échelles sont-elles scientifiquement valides ?**")
    st.write("L’évaluation de l’effort perçu est un outil validé scientifiquement depuis plusieurs décennies. De nombreuses études ont montré une forte corrélation entre les scores de RPE ou de CR10 et des mesures physiologiques objectives comme la consommation d’oxygène (VO2), la concentration de lactate sanguin et la fréquence cardiaque. Dans le football, des travaux comme ceux de Foster (1998) ou Malone et al. (2017) ont démontré que le suivi du CR10 permet de mieux prévenir les blessures en contrôlant les fluctuations de la charge d’entraînement.")
    st.write("Ces échelles sont aujourd’hui utilisées par la majorité des clubs professionnels, car elles offrent une méthode simple et efficace pour suivre la charge de travail sans nécessiter de matériel coûteux.")

    st.subheader("**Utilisation pratique et avantages**")
    st.write("L’évaluation de l’effort perçu permet d’ajuster les charges d’entraînement en fonction des besoins des joueurs. Elle est particulièrement utile pour :")
    st.write("- Adapter la planification des séances en fonction de l’état de forme des joueurs.")
    st.write("- Prévenir la fatigue excessive et le surentraînement en identifiant les périodes de surcharge.")
    st.write("- Optimiser la récupération en ajustant le volume et l’intensité du travail.")
    st.write("- Comparer les charges entre joueurs et ajuster les entraînements en fonction des postes.")
    st.write("Cet outil simple permet un suivi efficace de la charge d’entraînement et s’intègre facilement dans la gestion quotidienne d’un effectif.")
...
def take_test():
    ...
    st.header(translations["Sélectionnez un profil"][st.session_state.language])
    profile_options = list(st.session_state.profiles.keys())
    ...
    st.header(translations["Sélectionnez le type de test"][st.session_state.language])
    ...
    responses["Physique"] = st.select_slider(
        translations["À quel point cette séance/course a-t-elle été physiquement exigeante ?"][st.session_state.language],
        options=rpe_scale,
        key="physique"
    )
    ...
    responses["Mental"] = st.select_slider(
        translations["À quel point cette séance/course a-t-elle été mentalement exigeante ?"][st.session_state.language],
        options=rpe_scale,
        key="mental"
    )
    ...
    responses["Global"] = st.select_slider(
        translations["Comment évalues-tu globalement la difficulté de cette séance/course ?"][st.session_state.language],
        options=rpe_scale,
        key="global"
    )
...
# streamlit run "C:\Users\Utilisateur\Documents\Test_Rpe_aviron\RPE_test_Aviron.py"