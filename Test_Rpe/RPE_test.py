...
def show_explanation():
    ...
    st.subheader("**Comment utiliser le RPE ou le CR10 en football ?**")
    st.write("À la fin d’une séance d’entraînement ou d’un match, chaque joueur est invité à donner une note sur l’échelle choisie en répondant à une question simple : 'Comment as-tu ressenti l’intensité de cette séance ?' Le joueur doit donner une réponse en fonction de son ressenti global, sans se baser uniquement sur sa fatigue musculaire ou son essoufflement. L’idée est d’avoir une vision subjective de la difficulté de la séance, intégrant à la fois la charge physique, la charge mentale et les exigences du jeu. Cette évaluation est ensuite multipliée par la durée de l’effort en minutes pour obtenir une valeur de charge d’entraînement.")

    st.subheader("**Exemple de calcul :**")
    st.write("Un joueur attribue une note de 7 sur le CR10 après un match de 90 minutes. La charge d’entraînement est donc : 7 × 90 = 630. Ce chiffre permet de comparer les charges de travail entre les joueurs et sur différentes périodes.")
    ...
    st.write("Les données peuvent être utilisées individuellement ou collectivement. L’analyse des charges entre les différents postes permet également d’adapter le travail spécifique des joueurs selon leur rôle sur le terrain.")
